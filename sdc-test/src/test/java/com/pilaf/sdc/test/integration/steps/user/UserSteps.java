...
public class UserSteps implements Constants {
    ...
    private UserRegisterJSON userJson;
	...
	userJson = new UserRegisterJSON(login, password, firstName, surname, null, null,
	...
	ResponseEntity<UserDO> userResponse = new TestRestTemplate()
		.getForEntity(LOCALHOST + USER_PORT + "/user/byLogin/" + login, UserDO.class);
	UserDO user = userResponse.getBody();
	ActivationCodeJSON codeJson = new ActivationCodeJSON();
	codeJson.setUserId(user.getId());
	codeJson.setActivationCode(activationCode);
